...
class MediaStorage():
    ...
    def upload(self, file_path, content):
        if ENABLE_IAM_ROLE:
            ...
            s3 = boto3.resource(service_name='s3', region_name=REGION_NAME,aws_access_key_id=S3_ACCESS_ID,aws_secret_access_key= S3_ACCESS_KEY,endpoint_url=ENDPOINT_URL)
    ...
    def generate_signed_url_for_read(self, filepath, content_type, expiration):
        ...
        if ENABLE_IAM_ROLE:
            s3_client = boto3.client('s3', config=Config(signature_version='s3v4'), region_name=S3_REGION_NAME,endpoint_url=ENDPOINT_URL)
        else:
            s3_client = boto3.client('s3', config=Config(signature_version='s3v4'), region_name=S3_REGION_NAME,aws_access_key_id=S3_ACCESS_ID,aws_secret_access_key= S3_ACCESS_KEY,endpoint_url=ENDPOINT_URL)
        try:
            try:
                ...
                if ENABLE_IAM_ROLE:
                    ...
                    s3_client = boto3.client('s3', config=Config(signature_version='s3v4'), region_name=S3_REGION_NAME,aws_access_key_id=S3_ACCESS_ID,aws_secret_access_key= S3_ACCESS_KEY)
